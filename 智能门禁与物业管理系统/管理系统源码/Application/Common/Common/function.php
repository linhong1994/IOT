...
//天下畅通短信发送接口 $acc账号信息  $content内容  $mobile手机号
function sendSMS($content,$mobile){
	$acc=C("SMS_CONFIG");
	$post_data = array();
	$post_data['userid'] = $acc["userid"];//企业ID
	$post_data['account'] =$acc["account"];//账号
	$post_data['password'] = $acc["passwork"];//密码
	$post_data['content'] = $content.$acc["signed"]; //发生内容
	$post_data['mobile'] = $mobile;//手机号
	$url='http://xtx.telhk.cn:8080/sms.aspx?action=send1234';
...
//	print_r($result);
	return 'OK';$result["returnsms"]["message"];
}
...
function SMSVerify($mobile){
	...
	//判断一分钟内是否发送过验证码
	if(cookie("SMSVerifyExpire")){
		echo cookie("SMSVerifyExpire");
	...
	//验证码有效期为三十分钟
	$rand=123456;//rand(100000, 999999);
	$SMSVerifyCode['Code']=$rand;
	$SMSVerifyCode['mobile']=$mobile;
	cookie("SMSVerifyCode",$SMSVerifyCode,1800);
	$content="Q小区验证码:".$rand."。Q小区给您全新智能化住宅小区服务体验（服务热线：0595-22539376）";
	
	$returnValue=sendSMS($content,$mobile);
	
	//验证码发送成功后 一分钟内不可再次发送
	if($returnValue=="ok"){
		cookie("SMSVerifyExpire",1,60);
	}
	return $returnValue;
}

//验证短信验证码
function SMSCheck($mobile,$VerifyCode){
	if(cookie("SMSVerifyCode")){
		$SMSVerifyCode=cookie("SMSVerifyCode");
		if($mobile=$SMSVerifyCode["mobile"] && $VerifyCode==$SMSVerifyCode["Code"]){
//			cookie("SMSVerifyCode",NULL);
			return true;
		}
	}
	return false;
...
//通过token获取appid和secret
function token_to_appid_and_secret($token){
	$map["token"]=$token;
	return M("public")->where($map)->field("token,appid,secret")->find();
}


//通过token获取用户openid
function tokenGetOpenid($token="",$code=""){
	if(session("token")){
		$token=session("token");
	}
	if(!$token || $token=="-1"){
		if(!I("token")){
			header("location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$data["appid"]."&redirect_uri=".GetCurUrl()."&response_type=code&scope=snsapi_base&state=".I("state")."#wechat_redirect");
			return false;
		}
		$token=I("token");
	}
	...
	$data=token_to_appid_and_secret($token);
	...
	$url="https://api.weixin.qq.com/sns/oauth2/access_token?appid=".$data["appid"]."&secret=".$data["secret"]."&code=".$code."&grant_type=authorization_code";
	$redata=json_decode(curl($url),true);
//	print_r($redata);
	if(!$redata["openid"]){
		header("location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$data["appid"]."&redirect_uri=".GetCurUrl()."&response_type=code&scope=snsapi_base&state=".I("state")."#wechat_redirect");
	}
	
	return $redata["openid"];
}
